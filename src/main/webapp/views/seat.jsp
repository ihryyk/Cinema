...
<head>
    ...
    <style>
        ...
        <%@include file="styles/seat.css"%>
    </style>
</head>
<body>
<main class="movie_main">
    <div class="main_movieWrapper">
        <div>
            <img
                    src="data:image/png;base64, ${requestScope.session.movie.base64ImagePoster}"
                    width="240"
                    height="300"
                    class="movie_img"
            />
            <p class="descr movie_text">
                ${requestScope.session.movie.movieDescriptionList.get(0).title}
            </p>
        </div>
        <div class="movie_content">
            <div>
                ...
                <span class="movie_text"
                >${requestScope.session.endTime.getHours()}:${requestScope.session.endTime.getMinutes()}</span
                >
            </div>
            <p class="movie_text">
            ...
            <p class="movie_text"><fmt:message key="Price"/>: ${requestScope.session.price} <fmt:message
                    key="Money"/></p>
        </div>
    </div>
    <div class="movie_seats">
        <h2 class="movie_header"><fmt:message key="Seats"/></h2>
        <div class="movie_seat_list">
                ...
                <c:if test="${not empty sessionScope.user and sessionScope.user.role=='USER'}">
                    <c:if test="${entry.getValue()}">
                        <form
                                ...
                                method="get"
                                class="movie_seat_form"
                        >
                            <input
                                    type="hidden"
                                    name="command"
                                    value="TICKET_FORMATION_PAGE"
                            />
                            <input
                                    type="hidden"
                                    name="seatId"
                                    value="${entry.getKey().id}"
                            />
                            <input
                                    type="hidden"
                                    required
                            ...
                            >
                                <g transform="translate(1 1)">
                                    <g>
                                        <g>
                                            <path
                                                    d="M442.733,204.653h-57.398l-7.455-135.68C376.173,29.72,343.747-1,304.493-1h-98.987
                                        ...
                                        h37.547V348.867z M442.733,237.933h-55.467l-0.853-17.067h56.32V237.933z"
                                            />
                                            <path
                                                    d="M173.933,79.213c5.12,0.853,9.387-2.56,9.387-7.68c0.853-11.947,10.24-21.333,22.187-21.333h98.987
                            ...
                            <input
                                    type="submit"
                                    value="${entry.getKey().row}/${entry.getKey().number}"
                                    class="movie_seat_btn"
                            /></form
                        >
                    </c:if>
                    <c:if test="${not entry.getValue()}">
                        ...
                        movie_seat_form">
                            ...
                            <p class="movie_busy_seat_text">${entry.getKey().row}/${entry.getKey().number}</p>
                        </div>
                    </c:if>
                </c:if>
                <c:if test="${empty sessionScope.user}">
                    <c:if test="${entry.getValue()}">
                        <div class="movie_seat_form">
                            <svg
                                    fill="#3B945E"
                                    height="64px"
                                    width="64px"
